...
palancas$lag2   <- TRUE    # PRENDÕ ESTA PALANCA
palancas$delta2 <- TRUE    # PRENDÕ ESTA PALANCA
palancas$lag3   <- FALSE
...
# VARIABLES ALEX
  ...
  dataset[ , ALEX_mrentabilidad_mult := mrentabilidad*100]
  dataset[ , ALEX_mrentabilidad_log := log(mrentabilidad)]
  dataset[ , ALEX_mrentabilidad_bin := ifelse(mrentabilidad < 0,0,1)]
  dataset[ , ALEX_mrentabilidad_vol := ifelse(mrentabilidad < 0,mrentabilidad*(-1),mrentabilidad)]
  
  dataset[ , ALEX_mcuentas_saldo_mult := mcuentas_saldo*100]
  dataset[ , ALEX_mcuentas_saldo_log := log(mcuentas_saldo)]
  dataset[ , ALEX_mcuentas_saldo_bin := ifelse(mcuentas_saldo < 0,0,1)]
  dataset[ , ALEX_mcuentas_saldo_vol := ifelse(mcuentas_saldo < 0,mcuentas_saldo*(-1),mcuentas_saldo)]
  
  dataset[ , ALEX_mpayroll_log := log(mpayroll)]
  dataset[ , ALEX_mpayroll_bin := ifelse(mpayroll < 0,0,1)]
  dataset[ , ALEX_mpayroll_vol := ifelse(mpayroll < 0,mpayroll*(-1),mpayroll)]
  
  dataset[ , ALEX_cpayroll_trx_mult := cpayroll_trx*100]
  dataset[ , ALEX_cpayroll_trx_log := log(cpayroll_trx)]
  dataset[ , ALEX_cpayroll_trx_bin := ifelse(cpayroll_trx < 0,0,1)]
  dataset[ , ALEX_cpayroll_trx_vol := ifelse(cpayroll_trx < 0,cpayroll_trx*(-1),cpayroll_trx)]
  
  dataset[ , ALEX_mcuenta_debitos_automaticos_mult := mcuenta_debitos_automaticos*100]
  dataset[ , ALEX_mcuenta_debitos_automaticos_log := log(mcuenta_debitos_automaticos)]
  dataset[ , ALEX_mcuenta_debitos_automaticos_bin := ifelse(mcuenta_debitos_automaticos < 0,0,1)]  
  ...
  dataset[ , ALEX_mtransferencias_recibidas_mult := mtransferencias_recibidas*100]
  dataset[ , ALEX_mtransferencias_recibidas_log := log(mtransferencias_recibidas)]
  dataset[ , ALEX_mtransferencias_recibidas_bin := ifelse(mtransferencias_recibidas < 0,0,1)]
  ...
  dataset[ , ALEX_mextraccion_autoservicio_mult := mextraccion_autoservicio*100]
  dataset[ , ALEX_mextraccion_autoservicio_log := log(mextraccion_autoservicio)]
  dataset[ , ALEX_mextraccion_autoservicio_bin := ifelse(mextraccion_autoservicio < 0,0,1)]
  ...
  dataset[ , ALEX_prom_5_var_mult := ALEX_prom_5_var*100]  
  
  #Promedio de donde se mueve el dinero DEL VOL⁄MEN
  #OJO: Ac· hay una buena.
  ...
  dataset[ , ALEX_prom_5_var_vol_mult := ALEX_prom_5_var_vol*100]    
  
  # Master_status y Visa_status  
  # { 0,  6, 7, 9 }   indica el estado de la cuenta de la tarjeta de crÈdito. 
  ...
  # Si las junto, y las acomodo logarÌtmicamente, puede clasificar.
  dataset[ , ALEX_MyV_status_prom := (Master_status + Visa_status)/2]
  dataset[ , ALEX_MyV_status_prom_log := (log(Master_status + Visa_status)/2)]
  dataset[ , ALEX_MyV_status_prom_mult := (log(Master_status + Visa_status)/2)*100]
  
  # cpayroll_trx
  # Cantidad de Acreditaciones de Haberes en relaciÛn de depencia que 
  # le hicieron al cliente en ese mes.  
  # Un cliente puede estar en relacion de dependencia con mas de una empresa. 
  # Una empresa puede hacerle VARIOS depÛsitos al mismo empleado durante el mes.  
  ...
  # Monto total de los consumos efectuados durante el mes con la tarjeta de crÈdito
  # mtarjeta_visa_consumo y mtarjeta_master_consumo
  # Valores absolutos para testear vol˙men
  ...
  # Monto del pago minimo necesario para no ser moroso de la tarjeta de crÈdito
  ...
  #mcaja_ahorro_dolares	pesos	Monto total de las cajas de ahorro en dÛlares.  
  #El valor esta expresado en pesos, y se considera el valor del dolar de cierre del ˙ltimo dia h·bil del mes.
...
correr_todo  <- function( palancas )
{
  ...
  fwrite( ds_view,
        paste0( "./datasets/dataset_epic_view_", palancas$version, ".csv" ),
        logical01 = TRUE,
        sep= "," )
  View(ds_view)
}
